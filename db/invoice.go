...
func (i *InvoiceDB) GetSubFromStripeID (
	...
	err := i.DB.QueryRow(`SELECT sub_id FROM subscription WHERE stripe_sub_id=$1`, subStripeId).Scan(&sub.ID)
	if err != nil {
		return nil, err
	}
...
func (i *InvoiceDB) InsertInvoice (
	...
	_, err := i.DB.Exec(`INSERT into invoice 
		(
			...
			paid, status, attempted, total, created, invoice_url, app_fee_amt, default_payment_method, sub_id
		)
		...
		invoice.InvoiceURL, invoice.ApplicationFeeAmt, invoice.DefaultPaymentMethod, invoice.SubID,
	)
